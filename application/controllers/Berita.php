...
class Berita extends CI_Controller {
	...
	public function ajax_list()
   {
      ...
      foreach($list as $li){
         ...
         $row[] = 
         '<a class="btn btn-sm btn-warning text-white" href="'.base_url("index.php/berita/edit/$li->id").'" 
         title="Edit">
	...
	public function add()
	{
		...
		if($this->form_validation->run() == false){
			...
			$data['form_action'] = base_url("index.php/berita/add");
			$data['input'] 		= $input;
			$this->load->view('back/layouts/main', $data);
		}else{
			...
			redirect(base_url('index.php/berita'));
		}
	}
	...
	public function edit($id)
	{
		...
		if($this->form_validation->run() == false){
			...
			$data['page']			= 'berita/form';
			$data['input']			= $input;
			$data['form_action']	= base_url('index.php/berita/edit/' . $id);
			...
			if(!empty($_FILES['photo']['name'])){
				...
				if($upload){
					...
					redirect(base_url("index.php/berita/edit/$id"));
				}
			}
